...
```
  This script aims to plot out the city maps (with 5 example cities) from three different data sources for intuiitive comparasion. 
  
  This aids us to discuss and determine which data source to use. 
...
for (city.i in cities) {
  ...
  map <- 
    tm_shape(met.i) +
    tm_borders(col = "black", lwd = 2.5, fill_alpha = 0.6) +
    tm_shape(ua.i) +
    tm_borders(col = "blue", lwd = 1.5, fill_alpha = 0.6) +
    tm_shape(place.i) +
    tm_borders(col = "red", lwd = 1, fill_alpha = 1) +
